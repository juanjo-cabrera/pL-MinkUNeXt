...
def process_pcd(lama_model, image_path, src_color_dir, depth_dir, pcd_dir, device):
    ...
    depth_image_path = image_path.replace(src_color_dir, depth_dir) 
    image_color = cv2.imread(image_path, cv2.IMREAD_UNCHANGED) 
    image_depth = cv2.imread(depth_image_path, cv2.IMREAD_UNCHANGED)   
    ...
    print(f"Point cloud processed and saved in {new_pcd_path}")

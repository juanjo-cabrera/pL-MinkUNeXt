...
def process_image_and_pcd(model, lama_model, image_path, src_color_dir, depth_dir, pcd_dir, device):
    ...
    #image_depth = get_depth_v2(image_color)
    image_depth = model.infer_image(image_color)  # HxW raw depth map
    ...
    for root, dirs, files in os.walk(src_color_dir):
        ...
        for file in files:
            ...
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                process_image_and_pcd(model, lama_model, src_file_path, src_color_dir, depth_dir, pcd_dir, device)
...
if __name__ == "__main__":
    ...
    for environment in environments:
        ...
        for seq in map_sequences + query_sequences:
            os.makedirs(os.path.join(os.path.dirname(seq), 'depth'), exist_ok=True)


        # Directorio fuente y destino
        src_color_directory  = '/media/arvc/DATOS1/Marcos/DATASETS/COLD/' + environment + '/'

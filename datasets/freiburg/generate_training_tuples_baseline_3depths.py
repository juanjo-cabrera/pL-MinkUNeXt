...
def generate_pickle_3depths(run_folder, pickle_filename):
    print('Dataset root: {}'.format(PARAMS.dataset_folder))
    base_path_large = '/media/arvc/DATOS/Juanjo/Datasets/PCD_non_metric_Friburgo/'
    base_path_base = '/media/arvc/DATOS/Juanjo/Datasets/PCD_non_metric_Friburgo_base/'
    base_path_small = '/media/arvc/DATOS/Juanjo/Datasets/PCD_non_metric_Friburgo_small/'

    all_room_folders = sorted(os.listdir(os.path.join(base_path_large, run_folder)))
   
    folder_dir_large = os.path.join(base_path_large, run_folder)
    df_locations_large = get_pointcloud_positions(folder_dir_large, all_room_folders)

    folder_dir_base = os.path.join(base_path_base, run_folder)
    df_locations_base = get_pointcloud_positions(folder_dir_base, all_room_folders)
    # filtra df_locations_base para que solo se cojan 1 de cada 5 filas
    df_locations_base = df_locations_base.iloc[::5, :]

    folder_dir_small = os.path.join(base_path_small, run_folder)
    df_locations_small = get_pointcloud_positions(folder_dir_small, all_room_folders)
    # filtra df_locations_small para que solo se cojan 1 de cada 5 filas
    df_locations_small = df_locations_small.iloc[::5, :]

    df_locations = pd.concat([df_locations_large, df_locations_base, df_locations_small], ignore_index=True)

    print("Number of pointclouds: " + str(len(df_locations['file'])))

    construct_query_dict(df_locations, base_path_large + run_folder, pickle_filename, ind_nn_r=PARAMS.positive_distance, ind_r_r=PARAMS.negative_distance)

if __name__ == '__main__':
    TRAIN_FOLDER = PARAMS.train_folder
    VAL_FOLDER = PARAMS.val_folder
    generate_pickle_3depths(TRAIN_FOLDER, "training_queries_baseline_3depths.pickle")

...
sys.path.append('/home/arvc/Juanjo/develop/others_work/ml-depth-pro')
import depth_pro
...
def process_image_and_pcd(image_path, src_color_dir, depth_dir, pcd_dir, model, transform):
    ...
    image_depth = depth.squeeze().cpu().numpy()
    ...
    print(f"Nube procesada y guardada en {new_pcd_path}")
    # except Exception as e:
    #     print(f"Error al procesar la nube {image_path}: {e}")
...
def process_image(image_path, src_dir, dst_dir, model, image_processor):
    #plt.switch_backend('TkAgg')  # Specify the backend
    """
    Procesa la imagen y la guarda en la misma ubicación
    con '_depth' agregado al nombre del archivo.
...
def copy_structure_and_process_images(src_color_dir, depth_dir, pcd_dir, exclude_dir_names, model, image_processor):
    """
    ...
    for root, dirs, files in os.walk(src_color_dir):
        ...
        for file in files:
            ...
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                process_image(src_file_path, src_color_dir, depth_dir, model, image_processor)
...
if __name__ == "__main__":
    ...
    src_color_directory  = '/media/arvc/DATOS/Marcos/DATASETS/COLD/FRIBURGO_A/'
    pcd_directory = '/media/arvc/DATOS/Juanjo/Datasets/PCD_non_metric_Friburgo_DepthPro/'
    depth_directory = '/media/arvc/DATOS/Juanjo/Datasets/DepthPro_Friburgo/'
    exclude_directory_names = ['RepImages', 'RepresentativeImages']
    ...
    copy_structure_and_process_images(src_color_directory, depth_directory, pcd_directory, exclude_directory_names, model, image_processor)

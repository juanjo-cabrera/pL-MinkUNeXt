...
def plot_pcds_and_positions(df, df_database, dataset_path, output_path):
    ...
    for index, row in df.iterrows():
        if i % k == 0:
            ...
            fig.suptitle(f"Test vs Mapa (Índice: {i})", fontsize=24)

            # Plot images
            axes[0, 0].imshow(query_image)
            axes[0, 0].set_title("Nube de test", fontsize=20)
            axes[0, 0].axis('off')

            axes[0, 1].imshow(retrieved_image)
            axes[0, 1].set_title("Nube del mapa predicha", fontsize=20)
            axes[0, 1].axis('off')

            axes[1, 0].imshow(real_image)
            axes[1, 0].set_title("Nube del mapa más cercana", fontsize=20)
            axes[1, 0].axis('off')

            # Plot positions on the map
            axes[1, 1].scatter(df_database['x'], df_database['y'], color='blue', label="Posiciones del mapa", s=4)
            # axes[1, 1].scatter(row['query_x'], row['query_y'], color='red', label="Query Position")
            # dibuja una cruz en la posicion de la query
            axes[1, 1].scatter(row['query_x'], row['query_y'], color='red', label="Posición test", marker='x', s=225, linewidths=4)
            axes[1, 1].scatter(row['retrieved_database_x'], row['retrieved_database_y'], color='orange', 
                  marker='o', s=150, label="Posición predicha")
            axes[1, 1].scatter(row['real_database_x'], row['real_database_y'], color='green', 
                  marker='o', s=150, facecolors='none', edgecolors='green', linewidths=4, label="Posición más cercana")
            axes[1, 1].set_title("Posiciones en el mapa", fontsize=20)
...
if __name__ == "__main__":
    ...
    output_path_base = '/media/arvc/DATOS/Juanjo/Datasets/COLD/VISUAL_RESULTS_CASTELLANO/'

...
def do_train(model, dataloaders, evaluation_set_fa, evaluation_set_fb, evaluation_set_sa, evaluation_set_sb):
    ...
    s = get_datetime()
    model_name = 'casspr_pos_per_query'+ str(PARAMS.positives_per_query) + 'batch_size' + str(PARAMS.batch_size) + '_truncated_aug' + str(PARAMS.aug_mode) + 'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height) + '_' + s
    ...
    if PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_base/':
        wandb.init(project='casspr_base_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    elif PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_small/':
        wandb.init(project='casspr_small_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    else:
        wandb.init(project='casspr_experiment_truncated', config=params_dict, name= 'aug' + str(PARAMS.aug_mode) +'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    ###########################################################################
    #
    ###########################################################################
    ...
    for epoch in tqdm.tqdm(range(1, PARAMS.epochs + 1)):
        ...
        if 'val' in phases:
            ...
            if epoch == 50 or epoch == 1:
                ...
                file_name = '/home/arvc/Juanjo/develop/DepthMinkUNeXt/casspr/results.txt'
                model.eval()
                model.to(device)
...
from casspr.misc.utils import MinkLocParams
from casspr.models.model_factory import model_factory
import argparse
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train casspr model')
    parser.add_argument('--config', type=str, default='/home/arvc/Juanjo/develop/DepthMinkUNeXt/casspr/config_oxford_refined.txt', help='Path to configuration file')
    parser.add_argument('--model_config', type=str, default='/home/arvc/Juanjo/develop/DepthMinkUNeXt/casspr/model_config_oxford.txt', help='Path to the model-specific configuration file')
    ...
    # params = ModelParams(args.model_config)
    ...
    PARAMS.weights_path = '/home/arvc/Juanjo/develop/others_work/backup/CASSPR/weights/model_MinkFPNGeM_20250925_040746/epoch360.pth'

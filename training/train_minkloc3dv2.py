...
def do_train(model, dataloaders, evaluation_set_fa, evaluation_set_fb, evaluation_set_sa, evaluation_set_sb):
    ...
    s = get_datetime()
    model_name = 'minkloc3dv2_pos_per_query'+ str(PARAMS.positives_per_query) + 'batch_size' + str(PARAMS.batch_size) + '_truncated_aug' + str(PARAMS.aug_mode) + 'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height) + '_' + s
    ...
    if PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_base/':
        wandb.init(project='Minkloc3dv2_base_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    elif PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_small/':
        wandb.init(project='Minkloc3dv2_small_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    else:
        wandb.init(project='Minkloc3dv2_experiment_truncated', config=params_dict, name= 'aug' + str(PARAMS.aug_mode) +'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    ###########################################################################
    #
    ###########################################################################
    ...
    for epoch in tqdm.tqdm(range(1, PARAMS.epochs + 1)):
        ...
        if 'val' in phases:
            ...
            if epoch == 50 or epoch == 1:
                ...
                file_name = '/home/arvc/Juanjo/develop/DepthMinkUNeXt/minkloc3dv2/results.txt'
                model.eval()
                model.to(device)
...
from minkloc3dv2.model_factory import model_factory
import argparse
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_config', type=str, default='/home/arvc/Juanjo/develop/DepthMinkUNeXt/minkloc3dv2/minkloc3dv2.txt', help='Path to the model-specific configuration file')
    ...
    PARAMS.weights_path = '/home/arvc/Juanjo/develop/others_work/MinkLoc3Dv2/weights/minkloc3dv2_refined.pth'

...
def do_train(model, dataloaders, evaluation_set_fa, evaluation_set_fb, evaluation_set_sa, evaluation_set_sb):
    ...
    s = get_datetime()
    model_name = 'svtnet_pos_per_query'+ str(PARAMS.positives_per_query) + 'batch_size' + str(PARAMS.batch_size) + '_truncated_aug' + str(PARAMS.aug_mode) + 'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height) + '_' + s
    ...
    # Initialize wandb
    if PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_base/':
        wandb.init(project='svtnet_base_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    elif PARAMS.dataset_folder == '/media/arvc/DATOS1/Juanjo/Datasets/PCD_non_metric_Friburgo_small/':
        wandb.init(project='svtnet_small_experiment_truncated', config=params_dict, name='pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    else:
        wandb.init(project='svtnet_experiment_truncated', config=params_dict, name= 'aug' + str(PARAMS.aug_mode) +'pos' + str(PARAMS.positive_distance) + 'neg' + str(PARAMS.negative_distance) + 'voxel_size' + str(PARAMS.voxel_size) + 'height' + str(PARAMS.height))
    ###########################################################################
    #
    ###########################################################################
    ...
    for epoch in tqdm.tqdm(range(1, PARAMS.epochs + 1)):
        ...
        if 'val' in phases:
            ...
            if epoch == 50 or epoch == 1:
                ...
                file_name = '/home/arvc/Juanjo/develop/DepthMinkUNeXt/svtnet/results.txt'
                model.eval()
                model.to(device)
...
from minkloc3dv2.misc.utils import TrainingParams
from svtnet.models.model_factory import model_factory
import argparse
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train svtnet model')
    parser.add_argument('--config', type=str, default='/home/arvc/Juanjo/develop/DepthMinkUNeXt/minkloc3dv2/config_refined_v2.txt', help='Path to configuration file')
    parser.add_argument('--model_config', type=str, default='/home/arvc/Juanjo/develop/DepthMinkUNeXt/svtnet/models/minkloc3d_att.txt', help='Path to the model-specific configuration file')
    ...
    PARAMS.weights_path = '/home/arvc/Juanjo/develop/others_work/SVTNet/weights/model_MinkFPN_token_GeM_final.pth'
    # model.load_state_dict(torch.load(PARAMS.weights_path))

    checkpoint = torch.load(PARAMS.weights_path)

    # Remapear los nombres de las capas
    remapped_checkpoint = {}
    for key, value in checkpoint.items():
        # Reemplazar att_sa con asvt
        new_key = key.replace('backbone.att_sa', 'backbone.asvt')
        # Reemplazar att_token con csvt
        new_key = new_key.replace('backbone.att_token', 'backbone.csvt')
        remapped_checkpoint[new_key] = value

    # Cargar el checkpoint remapeado
    model.load_state_dict(remapped_checkpoint, strict=False)
    print("Model loaded with remapped weights")
    ...
    dataset_folders = ['/media/arvc/DATOS1/Juanjo/Datasets/COLD/PCD_DISTILL_ANY_DEPTH_LARGE/FRIBURGO_A/']

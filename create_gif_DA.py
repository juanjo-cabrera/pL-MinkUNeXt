...
def plot_pcds(df_database, dataset_path, output_path):
    ...
    """
    depth_estimators = ['PCD_DISTILL_ANY_DEPTH_LARGE', 'PCD_DISTILL_ANY_DEPTH_BASE', 'PCD_DISTILL_ANY_DEPTH_SMALL', 'PCD_LARGE', 'PCD_BASE', 'PCD_SMALL']
    i = 0
    row = df_database.iloc[i]
    images = []
    for depth_estimator in depth_estimators:    
        # check if the output figure already exists
        output_file_path = os.path.join(output_path, f'{i}_{depth_estimator}.png')
        ...
        image = get_pointcloud_image(pcd_path, output_file_path)
        images.append(image)

    # Opción 1: Crear GIF directamente con los frames (como ya lo haces)
    # images2gif(output_path, os.path.join(output_path, f'{i}.gif'), fps=6)
    
    # Opción 2: Crear video y luego convertirlo a GIF
    video_path = os.path.join(output_path, f'{i}.mp4')
    frames2video(output_path, video_path, fps=3)
    
    # Convertir video a GIF (después de crear el video)
    video2gif_ffmpeg(video_path, os.path.join(output_path, f'{i}_from_video.gif'), fps=3)
    
    print("Proceso completo: imágenes, video y GIF creados.")
